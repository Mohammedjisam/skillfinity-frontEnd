...
export default function ViewCourseDetails() {
  ...
  const toggleVisibility = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: isVisible
        ? "Hidden courses won't be visible to students."
        : "Unhidden courses will be visible to students.",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        await axiosInstance.put(`/user/data/togglecoursevisibility/${id}`, {
          isVisible: !isVisible,
        });
        setIsVisible(!isVisible);
        Swal.fire("Success!", `Course has been ${action}d.`, "success");
        toast.success(
  ...
  const formatDuration = (totalMinutes) => {
    if (!totalMinutes) return "Not specified";
          ...
          <div className="space-y-8">
            ...
            <Card>
              <CardContent className="p-6">
                <Button
                  onClick={viewLessons}
                  className="w-full text-lg py-6 mb-4 bg-blue-600 text-white hover:bg-blue-700"
                  size="lg"
                >
                  View Lessons
                </Button>
